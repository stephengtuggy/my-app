...
export class AnimalService {
    ...
    getAnimals(): Observable<Animal[]> {
        return this.http.get<Animal[]>(this.animalsUrl, {headers: this.httpOptions.headers, responseType: 'json'})
            .pipe(
                tap(animals => this.log(`fetched ${animals.length} animals`)),
    ...
    getAnimal(id: number): Observable<Animal> {
        ...
        return this.http.get<Animal>(url).pipe(
            tap(a => this.log(`fetched animal id=${id}. Its name is ${a.name}`)),
    ...
    addAnimal(animal: Animal): Observable<Animal> {
        return this.http.post<Animal>(this.animalsUrl, animal, this.httpOptions).pipe(
            tap((newAnimal: Animal) => this.log(`added animal w/ id=${newAnimal.id} and name ${newAnimal.name}`)),
    ...
    updateAnimal(animal: Animal): Observable<any> {
        ...
        return this.http.put(url, animal, this.httpOptions).pipe(
            tap(_ => this.log(`updated animal id=${animal.id}. New name ${animal.name}`)),
    ...
    deleteAnimal(animal: Animal | number): Observable<Animal> {
        ...
        return this.http.delete<Animal>(url, this.httpOptions).pipe(
            tap(animal => this.log(`deleted animal id=${id} (name=${animal.name})`)),
